 import React, { useState, useRef } from 'react'  // ✅ useState add kiya
...
console.log(error)
  ...
  return (
    <div className='w-full h-[100vh] bg-slate-200 flex flex-col justify-center items-center'>
      ...
      <div className='bg-white rounded-full border-4 border-[#20c7ff] shadow-gray-400 shadow-lg relative mb-[50px]' onClick={()=>image.current.click()}>
  ...
  <div className='absolute bottom-2 right-2 w-[40px] h-[40px] rounded-full bg-[#20c7ff] flex justify-center items-center shadow-md cursor-pointer  shadow-gray-400 '>
    <IoCameraOutline className='text-black text-[20px]'/>
  </div>

</div>
      <form className='w-[95%] max-w-[500px] flex flex-col gap-[20px] items-center justify-center'onSubmit={handleProfile}>
<input type="file" accept='image/*' ref={image} className='hidden'onChange={handleImage} />
        ...
        <button className='px-[20px] py-[10px] bg-[#20c7ff] rounded-2xl shadow-gray-400 shadow-lg text-[20px] w-[200px] mt-[20px] font-semibold hover:shadow-inner' disabled={saving}>{saving ? "Saving..." : "Save Profile"}</button>  {/* ✅ duplicate button hataya */}
      </form>
    </div>
  )
}
